...
local function state_machine()
   ...
   while true do 
	 ...
	 if response == false then
	    status = Status.error --Did not find
	 ...
	 else logger:warn("did not found network from +COPS")
	 end
	 ...
	 logger:warn("Error state..reseting modem")
	 power_toggle() -- Shut down modem
	 delay(5e6)     -- Wait for 5s for modem to be shut down
...
function wait(pattern, timeout)
   ...
   local counter = tmr.start(timer)
   repeat
      line = uart.read( UARTID, '*l', 1e3 ) -- Wait for 1ms to answer
